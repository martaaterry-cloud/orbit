...
function saveCheckin(){let d=load();d.checkins[dayKey()]={ts:Date.now(),mood,need:needToday.value.trim(),forMe:forMeToday.value.trim()};save(d);let got=awardDailyAction('checkin',.2,.2,'Check-in');toast(got?'Día guardado · +0,2':'Día actualizado');render()}
...
function saveQuickEntry(){let t=quickText.value.trim();if(!t)return toast('Escribe aunque sea una línea');let d=load(),p=prompts[new Date().getDate()%prompts.length];d.journal.push({id:uid(),ts:Date.now(),type:'rapida',title:p[0],text:t});save(d);quickText.value='';let got=awardDailyAction('journal',.1,.5,'Escribir');toast(got?'Guardado · +0,1':'Guardado');render()}
...
function saveJournalEntry(){let text=entryText.value.trim();if(!text)return toast('No hay nada que guardar todavía');let d=load();d.journal.push({id:uid(),ts:Date.now(),type:journalType,title:entryTitle.value.trim(),text,futureDate:journalType==='futuro'?(futureDate.value||null):null});save(d);entryTitle.value='';entryText.value='';futureDate.value='';let got=awardDailyAction('journal',.1,.5,'Escribir');toast(got?'Entrada guardada · +0,1':'Entrada guardada');render()}
...
function addGoodThing(){let t=goodThing.value.trim();if(!t)return toast('Escribe algo que quieras guardar');let d=load();d.goodThings.push({id:uid(),ts:Date.now(),text:t,meaning:goodMeaning.value.trim()});save(d);goodThing.value='';goodMeaning.value='';let got=awardDailyAction('goodThing',.1,.5,'Algo bueno');toast(got?'Guardado · +0,1':'Guardado en tu órbita de hoy');render()}
function deleteGood(id){let d=load();d.goodThings=d.goodThings.filter(x=>x.id!==id);save(d);render()}
